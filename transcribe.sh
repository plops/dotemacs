...
"$WHISPER_CLI_PATH" -m "$WHISPER_MODEL_PATH" -vm /home/kiel/src/whisper.cpp/models/ggml-silero-v6.2.0.bin --vad  -l "$LANGUAGE" -ovtt -osrt -f "$TEMP_AUDIO_FILE" -of "$BASENAME"
...
uv run ~/stage/dotemacs/vtt_to_lines.py "$BASENAME".vtt > "$BASENAME"_o.txt

echo exported "$BASENAME"_o.txt
